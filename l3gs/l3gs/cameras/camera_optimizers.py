...
class CameraOptimizer(nn.Module):
    ...
    def apply_to_camera(self, camera: Cameras) -> torch.Tensor:
        ...
        if self.num_cameras < camera_idx+1:
            diff = camera_idx+1 - self.num_cameras
            self.num_cameras = camera_idx+1
            self.pose_adjustment = torch.nn.Parameter(torch.cat([self.pose_adjustment.detach(), torch.zeros((diff, 6), device=self.pose_adjustment.device)]))
