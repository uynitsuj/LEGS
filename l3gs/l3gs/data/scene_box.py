...
class OrientedBox:
    ...
    def from_points(pts: Float[Tensor,"n 3"], device="cpu"):
        ...
        vec3d = o3d.utility.Vector3dVector(pts.detach().cpu().numpy())
        box = o3d.geometry.OrientedBoundingBox.create_from_points(vec3d)
        R = torch.tensor(box.R).to(device)
        T = torch.tensor(box.center).to(device)
        S = torch.tensor(box.extent).to(device)
        return OrientedBox(R=R,T=T,S=S)